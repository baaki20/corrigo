...
export default async function EvidencePage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ preview?: string }> }) {
  const { slug } = await params;
  const canPreview = (await searchParams).preview === "1" && Boolean(await getAdminSession());
  const dossier = await getDossier(slug, canPreview);
  if (!dossier) notFound();
