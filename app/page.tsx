...
export default async function Home() {
  ...
  return <main>
    ...
    <section><div className="section-heading"><div><span className="kicker">The latest files</span><h2>Open the case notes</h2></div><Link href="/library">View all →</Link></div><div className="dossier-grid">{dossiers.slice(0, 3).map((dossier, index) => <DossierCard dossier={dossier} index={index} key={dossier.id} />)}</div></section>
