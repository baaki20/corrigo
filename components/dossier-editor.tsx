...
export function DossierEditor({ initial, action, id }: { initial?: any; action: (formData: FormData) => void; id?: string }) {
  ...
  const [timing, setTiming] = useState(initial?.status || "DRAFT");
  ...
  return <form action={action} onSubmit={prepareSchedule} className="editor-grid">
    ...
    <div>
      ...
      <div className="editor-section"><span className="kicker">Publishing</span><div className="field"><label htmlFor="timing">Publication timing</label><select id="timing" name="timing" value={timing} onChange={(event) => setTiming(event.target.value)}><option value="DRAFT">Save as draft</option><option value="SCHEDULED">Schedule publication</option><option value="PUBLISHED">Publish now</option></select></div><div className="field"><label htmlFor="scheduledAt">Publish at <span className="field-hint">your local time</span></label><input id="scheduledAt" name="scheduledAt" type="datetime-local" required={timing === "SCHEDULED"} disabled={timing !== "SCHEDULED"} defaultValue={localDateValue(initial?.scheduledAt)} />{timing === "SCHEDULED" && <small className="field-help">The time is converted automatically and stored safely.</small>}</div></div>
